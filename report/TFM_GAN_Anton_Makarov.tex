...
El aprendizaje automático tiene como objetivo principal el desarrollo de técnicas que permitan aprender a las computadoras de manera autónoma, sin ser programadas explícitamente para ello, a partir de datos.

Dentro del aprendizaje automático existen problemas de muy diversa índole, una manera de clasificarlos es la basada en el tipo de información disponible.

\textbf{Aprendizaje supervisado.} Los problemas supervisados se caracterizan porque se dispone tanto una serie de muestras como de la información sobre cual debe ser el resultado para cada una de ellas. Algunos de los problemas típicos ante los que nos podemos encontrar dentro del aprendizaje supervisado son:
\begin{itemize}
	...
	\item Determinar si una imágen es un perro o un gato
\end{itemize}
...
\textbf{Aprendizaje no supervisado.} En el caso de los problemas no supervisados tan solo se dispone de las muestras, sin etiqueta alguna y se busca recuperar algún tipo de estructura de los datos. Algunos de los problemas típicos del aprendizaje no supervisado son:
\begin{itemize}
...
\end{itemize}
Algunos algoritmos propios del aprendizaje no supervisado son el K-means, 

\textbf{Aprendizaje por refuerzo.} El aprendizaje por refuerzo busca determinar la mejor acción a realizar en vista de un entorno determinado para maximizar la recompensa obtenida por dicha acción. Algunos problemas dentro de este tipo de aprendizaje son:
\begin{itemize}
...
Finalizamos esta introducción con un concepto algo menos usual en el ámbito del aprendizaje automático pero que nos será imprescindible para desarollar las redes generativas antagónicas.
\begin{defn}[Equilibrio de Nash]
	El equilibrio de Nash es una situación en la que ninguno de los jugadores tiene incentivo para cambiar su estrategia teniendo en cuenta las estrategias del resto de jugadores.
\end{defn}
\section{Redes neuronales}
Las redes neuronales son una clase de algoritmos de aprendizaje automático que en un principio fueron inspiradas por el estudio de las neuronas en el cerebro. Están formadas por una gran cantidad de unidades de procesamiento sencillas que funcionan con un grado de interconexión muy alto y se comunican de forma sencilla. Las redes neuronales se adaptan mediante la modificación de los pesos de las conexiones. Una red neuronal tiene 3 elementos básicos:

\begin{itemize}
	\item Caracterización de la neurona
	\item Definición de la topología de interconexión
	\item Definición de reglas para el ajuste de pesos
\end{itemize}

La caracterización de la neurona consta de la definición del vector de entrada, al que se aplica una serie de pesos. La topología define
...
La particularidad de las CNNs es que introducen una visión local del espacio. Pensemos en una imagen de un perro, parece razonable que intentemos identificar que efectivamente nos encontramos ante un perro y no un gato fijándonos en pequeñas partes de la imagen como ojos, hocico, patas, orejas, etc. en lugar de todos los píxeles a la vez sin ningún tipo de relación espacial entre si. Es destacable que este tipo de filtros ya se hacían antes de las redes convolucionales, seleccionando regiones de interés manualmente. Sin embargo, las redes convolucionales van más allá, automatizando el proceso de extracción de características, aprendiendo durante el entrenamiento los filtros más idóneos.
...
En la Figura tenemos una representación esquemática de todos los elementos involucrados. En la parte derecha está representada en color azul una imagen de $5\times 5$ píxeles en $3$ canales: rojo, verde y azul. Los bordes de color gris son un relleno (\textbf{Padding} en inglés) de ceros cuya función es recoger mejor la información de los bordes de la imagen y modular las dimensiones de salida. La imagen está siendo recorrida por una cuadrícula $3 \times 3$ con un desplazamiento (\textbf{Stride} en inglés) de tamaño $2$, es decir, en cada paso se mueve dos posiciones hacia la derecha y al llegar al final de la línea baja dos posición, colocándose de nuevo en la izquierda. En la parte central se sitúan los filtros, en este caso dos, que generan de este modo dos mapas de características distintos, representados a la izquierda. Estos filtros funcionan de la manera siguiente: primero se realiza un producto elemento a elemento entre la cuadrícula y el filtro en cada canal de color y después se suman los resultados, dando lugar a un elemento del mapa de características.

En general se puede calcular la dimensión de salida de una capa convolucional mediante la siguiente fórmula:
\begin{equation*}
O = \frac{W-K+2P}{S}+1,
\end{equation*}
donde $W$ es el tamaño de entrada, $K$ el tamaño del filtro, $P$ el padding y $S$ la longitud del paso. En nuestro ejemplo por tanto tendremos un tamaño de salida $O = \frac{5-3+2}{2}+1 = 3$ y, como hemos utilizado dos filtros, tendrá profundidad $2$, en notación común $3\times 3\times 2$.
