...
\section{DCGAN}
Las redes convolucionales generativas antagónicas profundas son una extensión de las GANs tradicionales. Una de sus características principales es el uso de redes convolucionales con arquitecturas relativamente sencillas para el discriminador y redes con convoluciones traspuestas para el generador. 

Las convoluciones traspuestas, también llamadas convoluciones fraccionales en algunos textos, tienen como objetivo aumentar la dimensión de los tensores de entrada. Antes de detallar su funcionamiento, vemos una manera sencilla de alcanzar este objetivo. Podríamos simplemente duplicar cada fila y columna según un factor. En la Figura \ref{duplicate} mostramos un ejemplo de esta técnica.
...
Aunque existen otras maneras algo más sofisticadas de aumentar el tamaño de los tensores, como por ejemplo la interpolación bilineal o bicúbica, nosotros nos vamos a centrar en la convolución fraccional, que permite que los parámetros que llevan a cabo la transformación sean aprendidos por la red generadora.

La convolución transpuesta posee los mismos parámetros que la convolución común. Una representación esquemática de la idea se puede ver en la Figura \ref{trans:conv}

\begin{figure}[h]
	\centering
	\includegraphics[width=\linewidth]{../images/report/transposed_conv.pdf}
	\caption{Aumento del tamaño de una imagen mediante convolución traspuesta.}
	\label{trans:conv}
\end{figure}

En el artículo \cite{radford2015unsupervised} los autores proporcionan una serie de recomendaciones empíricas para incrementar la estabilidad y obtener imágenes de mayor calidad. Los puntos más relevantes que se pueden extraer son:
