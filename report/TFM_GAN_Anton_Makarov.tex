...
En esta sección realizaremos un análisis de la teoría que hay detrás de las redes generativas antagónicas, utilizando como referencias principales \cite{goodfellow2014generative, goodfellow2016nips}. Mostraremos que el entrenamiento nos permite recuperar la distribución de los datos siempre y cuando $G$ y $D$ tengan capacidad suficiente. A lo largo de la sección supondremos que los modelos tienen capacidad infinita, estudiando la convergencia en el espacio de las funciones de densidad.

\section{El juego minimax tiene óptimo global para $p_g = p_d$}
Veamos en primer lugar cual es el discriminador óptimo para cualquier generador.
\begin{teorema}
...
\begin{proof}
	...
	\end{aligned}
	\end{equation}
Para todo par $(a,b) \in \mathbb{R}^2$ la función $f(y) = a\log(y) + b\log(1-y)$ alcanza el máximo en $\frac{a}{a+b}$ dentro del intervalo $[0,1]$, con lo que se obtiene el resultado.
\end{proof}
...
\begin{teorema}
	El mínimo global de $C(G)$ se alcanza si y solo si $p_g=p_d$, donde $C(G) = -\log 4$.
\end{teorema}
\begin{proof}
	Para $p_g = p_d$ se tiene que $D_G^* = \frac{1}{2}$. Introduciendo este valor en la Ecuación tenemos que
\begin{equation*}
...
\end{proof}
\section{Convergencia del Algoritmo \ref{alg:gan}}
\begin{teorema}
	Si $G$ y $D$ tienen suficiente capacidad y, en cada paso del Algoritmo \ref{alg:gan} se permite al discriminador alcanzar su óptimo para el generador dado, actualizando $p_g$ con el objetivo de mejorar el criterio:
	\begin{equation}
	contenidos...
	\end{equation}
