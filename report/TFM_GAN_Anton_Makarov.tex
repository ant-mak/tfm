...
\begin{abstractpage}
	\begin{abstractnew}{spanish}
		En este trabajo estudiamos en profundidad las Redes Generativas Antagónicas, un modelo generativo no supervisado que explora la idea de poner a competir entre sí a dos redes neuronales. Expandimos las demostraciones teóricas del artículo en las que fueron introducidas por primera vez \cite{goodfellow2014generative} y proponemos la utilización de una variante de este tipo de redes, llamada DCGAN \cite{radford2015unsupervised}, para la generación de imágenes de cuadros de arte, demostrando experimentalmente que es posible obtener imágenes visualmente atractivas mediante este método.
	\end{abstractnew}
	
	\begin{abstractnew}{english}
		In this dissertation we study Generative Adversarial Networks, an unsupervised generative model that explores the idea of setting two neural networks to compete with each other. We expand the proofs from the paper where they were first introduced \cite{goodfellow2014generative} and propose the use of a variant of these kind of networks called DCGAN \cite{radford2015unsupervised} to generate images of art, proving experimentally that it is possible to obtain visually appealing images with this method.
	\end{abstractnew}
\end{abstractpage}
...
La acogida de esta idea ha sido espectacular, llevando a la publicación de numerosos artículos con extensiones, mejoras y modificaciones, obteniendo resultados impresionantes. Entre dichos artículos consideramos que merecen una mención especial \emph{DCGAN} \cite{radford2015unsupervised}, que introduce una gran cantidad de mejoras sobre artículo original \cite{goodfellow2014generative} y describe el espacio latente de las GANs, dando la posibilidad de realizar operaciones aritméticas sobre los vectores de entrada, para así, por ejemplo, obtener una imagen de una mujer con gafas restando al vector de hombre con gafas el de hombre sin gafas y, después, al resultado sumarle el de mujer sin gafas. En la Figura \ref{DCGAN} podemos visualizar este proceso. \emph{Progressive Growing of GANs} \cite{karras2017progressive}, que permite generar imágenes de alta resolución mediante un entrenamiento progresivo, añadiendo capas a las redes a medida que se sucede el entrenamiento. En la Figura \ref{PROGAN} podemos ver una muestra de las imágenes generadas mediante esta técnica. \emph{Cycle-GAN} \cite{zhu2017unpaired}, que permite la traducción bidireccional de imágenes según su estilo, conservando el contexto. En la Figura \ref{CYCLEGAN} vemos cómo se transforman zebras en caballos (estilo) mientras que se mantiene la pradera (contexto). \emph{GAN text to image synthesis}, \cite{reed2016generative} que genera imágenes a partir de descripciones de texto. En la Figura \ref{text_image} pedimos en texto una flor blanca y amarilla, con pétalos finos de color blanco y estambre amarillo, obteniendo como resultado imágenes que cumplen las condiciones impuestas. Por último \emph{Wasserstein GAN} \cite{arjovsky2017wasserstein}, que describe una variante de GAN con muy buenas propiedades matemáticas, mejorando sobre todo la estabilidad y convergencia.
...
\begin{figure}[h]
	...
	\begin{subfigure}{.8\textwidth}
	\centering
	\includegraphics[width=.6\linewidth]{../images/report/text_to_image.png}
	\caption{GAN text to image synthesis}
	\label{text_image}
	\end{subfigure}
...
\end{itemize}
Algunos algoritmos utilizados en aprendizaje no supervisado son el K-means, el clústering jerárquico o las redes neuronales.
...
\begin{itemize}
	\item Enseñar a una máquina a jugar al ajedrez, Go, StarCraft, etc.
	\item Enseñar a una máquina a conducir.
\end{itemize}
Los algoritmos más conocidos dentro de esta área son el Q-learning, SARSA o DQN.
...
\begin{obs}
	El Perceptrón Multicapa, o \textbf{PMC}, es una de las arquitecturas de red neuronal más importantes, siendo empleada con frecuencia como parte de redes más complejas. No es más que una red neuronal con una capa de entrada, una de salida y al menos una capa oculta en la que todas las neuronas de una capa están conectadas con todas las de la siguiente.  
\end{obs}
...
O = \frac{W-K+2P}{S}+1,
\end{equation*}
donde $W$ es el ancho o alto, $K$ el tamaño del filtro, $P$ el padding y $S$ el stride. En nuestro ejemplo, por tanto, tendremos un tamaño de salida $O = \frac{5-3+2}{2}+1 = 3$. En general se suele utilizar más de un filtro en cada capa convolucional, obteniendo así también dimensiones de profundidad.
...
En este capítulo describiremos las ideas principales, tanto teóricas como prácticas de las redes generativas antagónicas. Daremos en primer lugar una idea conceptual de su funcionamiento, centrándonos después en la demostración de sus propiedades teóricas más relevantes. Finalmente concluimos con algunas consideraciones a tener en cuenta en la práctica. Una buena referencia que ofrece una visión general del tema es \cite{creswell2018generative}.
...
Traduzcamos ahora esta idea a términos matemáticos. Sea el generador $G$ un PMC con parámetros (pesos) $\theta_g$ que tiene por objetivo encontrar una distribución $p_g$ lo más parecida posible a la distribución de los datos $p_d$. Para ello toma un vector aleatorio $z$ distribuido según una cierta distribución $p_z$ y lo lleva al espacio definido por los datos, generando así una imagen falsa $G(z)$. Por otro lado, sea el discriminador $D$ otro PMC con parámetros $\theta_d$, que, para una muestra, devuelve la probabilidad de que esta sea real $(x)$ o falsa $(G(z))$. Entrenaremos $D$ para que maximice la probabilidad de asignar la etiqueta correcta tanto a los ejemplos de entrenamiento como a los generados, mientras que, al mismo tiempo, entrenaremos $G$ para que minimice $\log (1- D(G(z)))$. Es decir, tenemos el siguiente juego minimax con función de utilidad $V(G,D)$:
\begin{equation}
\min_G \max_D V(G,D) = \mathbb{E}_{x \sim p_d(x)} [\log D(x)] + \mathbb{E}_{z \sim p_z(z)} [\log (1- D(G(z)))].
\label{minimax}
\end{equation}

En la Figura \ref{ganconcept} mostramos una representación esquemática del funcionamiento de las GANs. En el caso de imágenes a color, podemos interpretar el esquema de la manera siguiente: por un lado, un vector aleatorio $z$ de dimensión $(1 \times 1 \times m)$ entra en la red generadora $G$, que devuelve una imagen falsa $G(z)$ de dimensión $(n \times n \times 3)$. Esta imagen pasa a la red discriminadora $D$, que devuelve la probabilidad $D(G(z))$ de que la imagen sea real. Por otro lado, tomamos una imagen $x$ (de dimensión $(n \times n \times 3)$) del conjunto de datos y la procesamos a través de la red discriminadora, obteniendo la probabilidad $D(x)$ de que la imagen sea, en efecto, real. En el Algoritmo \ref{alg:gan} describimos el proceso de manera más concisa, dando además, una primera idea de cómo se lleva a cabo el entrenamiento.
...
\begin{algorithm}[h]
	...
	\For{$k$ pasos}
	{
	Tomar muestra $(z^{(1)}, z^{(2)}, \ldots, z^{(m)})$ de tamaño $m$ de $p_z(z)$\;
	...
	Tomar muestra $(z^{(1)}, z^{(2)}, \ldots, z^{(m)})$ de tamaño $m$ de $p_z(z)$\;
	Actualizar el generador descendiendo su gradiente:
	$$
	\theta_g \leftarrow \theta_g - \alpha \nabla_{\theta_g} \frac{1}{m}\sum_{i = 1}^{m} \log \left( 1 - D(G(z^{(i)}))\right).
	$$
	}
	\caption{Entrenamiento minibatch de una red generativa antagónica. Los valores de $k$ y $\alpha$ son hiperparámetros que se pueden elegir libremente, $\theta_d$ y $\theta_g$ son los pesos de las redes $D$ y $G$ respectivamente.}
	\label{alg:gan}
\end{algorithm}
...
Veamos en primer lugar cuál es el discriminador óptimo para un generador dado.
\begin{teorema}
...
\begin{teorema}
	Si $G$ y $D$ tienen suficiente capacidad y en cada paso del Algoritmo \ref{alg:gan} se permite al discriminador alcanzar su óptimo para el generador dado, actualizando $p_g$ de manera que se mejore el criterio:
	\begin{equation*}
	\mathbb{E}_{x\sim p_d}[\log(D_G^*(x))] + \mathbb{E}_{x\sim p_g}[\log(1-D_G^*(x))],
	\end{equation*}
	lleva a que $p_g$ converge a $p_d$.
\end{teorema}
\begin{proof}
...
Por otro lado, durante el entrenamiento de las GANs, nos podemos encontrar con el fenómeno conocido como \emph{mode collapse}, que consiste en que el generador, una vez ha conseguido generar una muestra o grupo de muestras muy similares con las que es capaz de engañar al discriminador, acaba generando lo mismo una y otra vez, estancándose en el entrenamiento y produciendo resultados sin apenas diversidad. Para tratar de solventar este problema, se han propuesto diversas técnicas, como por ejemplo modificar la función de coste, incluyendo un término que obligue a la red generadora a aumentar la diversidad en las muestras generadas. Desafortunadamente, este tipo de métodos suelen comprometer la calidad de las muestras, introduciendo grandes cantidades de ruido. Recientemente se han publicado artículos que proponen nuevas metodologías de entrenamiento que prometen solucionar este problema sin perder calidad, por ejemplo MGGAN \cite{bang2018mggan}.

Finalmente, otro problema con el que nos encontramos al entrenar redes generativas antagónicas en la práctica, es que es muy complicado comparar los resultados obtenidos. En general, no hay una manera infalible de comprobar que las imágenes generadas por una red son mejores que las generadas por otra. Se han propuesto varias medidas que tratan de cuantificar de manera objetiva la calidad de las muestras generadas, pero por el momento es un tema de investigación activo. Un buen artículo que inspecciona el estado del arte es \cite{NIPS2018_7350}.

\chapter{Generación de imágenes artísticas}
...
Es importante no confundir las convoluciones fraccionales o traspuestas con las deconvoluciones, que tienen como objetivo recuperar la entrada de una convolución, es decir, invertir la operación de convolución. Una convolución fraccional tan solo busca recuperar las dimensiones originales. Para ello, realiza la convolución usual pero añadiendo ceros de manera ingeniosa para representar el stride y el padding. En la Figura \ref{frac:conv} describimos visualmente la manera de añadir dicho relleno de ceros. Es importante notar que la cuadrícula se mueve siempre como una convolución con stride $1$. Del mismo modo que en la convolución usual, podemos calcular la dimensión de salida de una capa de convolución fraccional mediante la siguiente fórmula:
\begin{equation*}
O = S(W-1)+K-2P,
\end{equation*}
donde $W$ es el ancho o alto, $K$ el tamaño del filtro, $P$ el padding y $S$ el stride.
...
\begin{itemize}
	...
	\item No utilizar capas totalmente conectadas (PMC).
	\item Utilizar funciones de activación ReLU en todas las capas del generador salvo en la última, en la que utilizar $\tanh$.
	\item Utilizar funciones de activación LeakyReLU en todas las capas del discriminador salvo en la última, en la que utilizar sigmoide.
...
Basándonos en las recomendaciones mencionadas anteriormente, hemos decidido utilizar la arquitectura que mostramos en la Figura \ref{architecture}, con una red convolucional sencilla para el discriminador y una red de convoluciones fraccionales para el generador. Además de las recomendaciones mencionadas anteriormente, hemos utilizado una técnica conocida como suavizado de etiquetas por un lado (one sided label smoothing en inglés \cite{salimans2016improved}). Consiste en modificar el valor de la etiqueta correspondiente a las imégenes reales de $1$ a $0.9$ para reducir la confianza de la red discriminadora al hacer las predicciones, ganando así estabilidad. Para consultar los detalles a más bajo nivel, recomendamos visitar la página correspondiente al código en el repositorio creado para este trabajo\footnote{\url{https://github.com/ant-mak/tfm/blob/master/src/tfm_teci_antonmakarov_gan-paintings.ipynb.}}.

\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{../images/report/architecture.pdf}
