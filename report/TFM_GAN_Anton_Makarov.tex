...
En este trabajo vamos a centrar nuestro estudio en las Redes Generativas Antagónicas (\textbf{GANs} o Generative Adversarial Networks en inglés), un tipo de modelo generativo que está recibiendo mucha atención en la actualidad debido a lo innovador de su idea y a la cantidad de aplicaciones que posee. Los modelos generativos tienen como objetivo describir la distribución subyacente de los conjuntos de datos con los que trabajan, por ejemplo podrían aproximar la distribución de las obras de arte para luego tomar muestras y crear cuadros totalmente nuevos. Las GANs fueron propuestas por Ian Goodfellow \cite{goodfellow2014generative} en 2014, introduciendo, como principal novedad, el concepto de equilibrio de teoría de juegos. Las GANs se fundamentan en encontrar un equilibrio en un juego en el que dos redes neuronales compiten entre sí, una generando imágenes a partir de ruido aleatorio y la otra decidiendo si son reales o falsas, en un entrenamiento en bucle.

La acogida de esta idea ha sido espectacular, llevando a la publicación de numerosos artículos con extensiones, mejoras y modificaciones, obteniendo resultados impresionantes. Entre dichos artículos consideramos que merecen una mención especial \emph{DCGAN} \cite{radford2015unsupervised}, que introduce una gran cantidad de mejoras al artículo de Goodfellow y describe el espacio latente de las GANs (Figura \ref{DCGAN}), Progressive Growing of GANs \cite{karras2017progressive}, que permite generar imágenes de alta resolución mediante el entrenamiento progresivo de las redes (Figura \ref{PROGAN}), \emph{Cycle-GAN} \cite{zhu2017unpaired}, que permite la traducción de imágenes según estilo, conservando el contexto (Figura \ref{fig:sfig1}) y \emph{Wasserstein GAN} \cite{arjovsky2017wasserstein} que describe una variante de GAN con muy buenas propiedades matemáticas. 
...
\begin{figure}[h]
	...
	\begin{subfigure}{.33\textwidth}
		...
		\label{CYCLEGAN}
	\end{subfigure}
	\caption{Imágenes descriptivas de las técnicas.}
	\label{GAN:papers}
\end{figure}

En el Capítulo 2 introduciremos los conceptos esenciales del aprendizaje automático, daremos algunas definiciones que serán de utilidad en capítulos posteriores y describiremos brevemente las redes neuronales, centrando la atención en las redes neuronales convolucionales. En el Capítulo 3 explicaremos en mayor detalle la idea conceptual de las redes generativas antagónicas y desarrollaremos la teoría matemática en la que se fundamentan. El Capítulo 4 está dedicado a la descripción de nuestra implementación en \texttt{Python} de una DCGAN para generar arte. Este capítulo va ligado al repositorio de código con el que acompañamos el proyecto\footnote{Todo el código de este proyecto se puede encontrar en el repositorio de GitHub \url{https://github.com/ant-mak/tfm}, donde se incluyen las instrucciones para la reproducción de los resultados del proyecto.} y constituye una parte fundamental del trabajo realizado. Finalmente, en el Capítulo 5, presentamos las conclusiones y señalamos algunos temas y líneas de investigación que sería interesante perseguir en el futuro.
...
En este capítulo describiremos las ideas principales, tanto teóricas como prácticas de las redes generativas antagónicas. Fueron propuestas por primera vez por Ian Goodfellow en 2014 \cite{goodfellow2014generative}, mezclando conceptos de aprendizaje automático no supervisado, supervisado y teoría de juegos. Desde entonces han suscitado una actividad investigadora muy importante, con aplicaciones fundamentalmente en el campo de la visión por computador.
...
\begin{equation}
\begin{aligned}
C(G) &= \int_x  (\log 2 - \log 2) p_d(x) + p_d(x) \log \left( \frac{p_d(x)}{p_g(x)+p_d(x)}\right) \\
&+ (\log 2 - \log 2) p_g(x) + p_g(x) \log \left( \frac{p_g(x)}{p_g(x)+p_d(x)}\right) \diff x.
\end{aligned}
\end{equation}
...
&+\int_x p_d(x) \left( \log 2 + \log \left( \frac{p_d}{p_g+p_d}\right) \right)\\
&+ p_g(x) \left( \log 2 + \log \left( \frac{p_g}{p_g+p_d}\right) \right) \diff x.
\end{aligned}
\end{equation}
Simplificando términos obtenemos:
\begin{equation}
-\log 4 + \int_x p_d(x)\log \left( \frac{p_d}{\frac{p_g+p_d}{2}}\right) \diff x + \int_x p_g(x)\log \left( \frac{p_g}{\frac{p_g+p_d}{2}}\right) \diff x.
\label{pre:KL}
\end{equation}
Ahora podemos identificar la ecuación \ref{pre:KL} como la divergencia de Kullback-Leibler:
\begin{equation}
C(G) = -\log 4 + D_{\mathrm{KL}}\left(p_d \Bigg \| \frac{p_d + p_g}{2}\right)+D_{\mathrm{KL}}\left(p_g \Bigg\| \frac{p_d + p_g}{2}\right).
\end{equation}
Podemos escribir la ecuación anterior utilizando la divergencia de Jensen-Shannon:
\begin{equation}
C(G) = -\log 4 + 2 D_{\mathrm{JS}}(p_d\|p_g).
\end{equation}
