\documentclass{beamer}

\mode<presentation>{
\usetheme{Madrid}
\setbeamertemplate{navigation symbols}{}
}

\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{animate}
\usepackage{xmpmulti}
\usepackage[spanish]{babel}
\usepackage[latin1]{inputenc}

%====================================

\title[Trabajo de fin de máster]{Redes Generativas Antagónicas}

\author[Antón Makarov]{Antón Makarov Samusev}
\institute[UCM/UPM]
{
Universidad Complutense de Madrid \\
\medskip
Universidad Politécnica de Madrid \\
\medskip
\textit{amakarov@ucm.es}
}
\date{25 de septiembre de 2019}

%====================================

\begin{document}

\begin{frame}
\titlepage
\end{frame}

%====================================

\begin{frame}
\frametitle{Índice}
\tableofcontents
\end{frame}

%====================================

%------------------------------------------------
\section{Redes Generativas Antagónicas}
%------------------------------------------------

\begin{frame}
	\frametitle{Descripción del problema}
	\begin{itemize}
	\item <1-> Goodfellow et. al. 2014
	\item <2-> Aprendizaje no supervisado
	\item <3-> Describir la distribución que siguen los datos
	\item <4-> Generar muestras a partir de dicha distribución
	\item <5->  Mediante redes neuronales que compiten entre sí
	\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Idea conceptual}
\begin{figure}
\includegraphics[width=0.8\linewidth]{../images/report/scheme.pdf}
\end{figure}
\end{frame}

\begin{frame}
\frametitle{Aspectos teóricos}
\begin{equation*}
\min_G \max_D V(G,D) = \mathbb{E}_{x \sim p_d(x)} [\log D(x)] + \mathbb{E}_{z \sim p_z(z)} [\log (1- D(G(z)))].
\label{minimax}
\end{equation*}
\end{frame}

%------------------------------------------------
\section{Generación de arte}
\subsection{DCGAN}
%------------------------------------------------

\begin{frame}
\frametitle{DCGAN}
\center \textbf{Deep Convolutional Generative Adversarial Network}
\vfill
\begin{itemize}
	\item Discriminador: Convolución con stride
	\item Generador: Convolución fraccional con stride
	\item No utilizar capas totalmente conectadas
	\item Generador: ReLU $+ \tanh$
	\item Discriminador: LeakyReLU $+$ sigmoide
	\item BatchNorm
	\item Inicialización gaussiana
	\item <2-> One sided label smoothing
\end{itemize}
\end{frame}

%------------------------------------------------
\subsection{Metodología}
%------------------------------------------------

\begin{frame}
\frametitle{Obtención y pre-procesado}
	\begin{itemize}
		\item Conjunto de datos obtenido de una competición de Kaggle
		\item Más de 100000 imágenes $\approx 50$ GB
		\item Algunas imágenes corruptas
		\item Escalado de tamaños y proporciones
		\item Normalización
		\item Carga como tensores
	\end{itemize}
\end{frame}

\begin{frame}
	\begin{figure}
		\includegraphics[width=0.7\linewidth]{../images/results/original_data.png}
	\end{figure}
\end{frame}

\begin{frame}
	\frametitle{Arquitectura}
	\begin{figure}
		\includegraphics[width=0.45\linewidth]{../images/report/architecture.pdf}
	\end{figure}
\end{frame}

%------------------------------------------------
\subsection{Resultados}
%------------------------------------------------

\begin{frame}
	\begin{figure}
		\includegraphics[width=0.7\linewidth]{../images/results/generated_data.png}
	\end{figure}
\end{frame}

\begin{frame}
	\centering
	\transduration<0-16>{0}
	\multiinclude[<+->][format=png, graphics={width=0.85\textwidth}]{../misc/something}
\end{frame}

%------------------------------------------------
\subsection{Recursos y rendimiento}
%------------------------------------------------

\begin{frame}
	\frametitle{Recursos y rendimiento}
	\begin{itemize}
		\item Imprescindible GPU para el entrenamiento
		\item 24 horas para 30 épocas
		\item en PC normal, 20 veces más lento
	\end{itemize}
\end{frame}

%------------------------------------------------
\section{Algunas arquitecturas basadas en GANs}
%------------------------------------------------

\begin{frame}
	\frametitle{Evolución en generación de caras}
	\begin{figure}
		\includegraphics[width=1\linewidth]{../images/presentation/evolution.png}
		\caption{Ian Goodfellow. \textit{4.5 years of GAN progress on face generation.}}
	\end{figure}
\end{frame}

\begin{frame}
	\frametitle{DCGAN}
	\begin{figure}
		\includegraphics[width=1\linewidth]{../images/presentation/dcgan.png}
	\end{figure}
\end{frame}

\begin{frame}
	\frametitle{Text to image}
	\begin{figure}
		\includegraphics[width=0.7\linewidth]{../images/presentation/text_to_image.png}
	\end{figure}
\end{frame}

\begin{frame}
	\frametitle{PGAN}
	\begin{figure}
		\includegraphics[width=1\linewidth]{../images/presentation/progan.png}
	\end{figure}
\end{frame}

\begin{frame}
	\frametitle{Pix2Pix}
	\begin{figure}
		\includegraphics[width=1\linewidth]{../images/presentation/pix2pix.png}
	\end{figure}
\end{frame}

\begin{frame}
	\frametitle{CycleGAN}
	\begin{figure}
		\includegraphics[width=1\linewidth]{../images/presentation/cyclegan.png}
	\end{figure}
\end{frame}

%------------------------------------------------
\section{Consideraciones prácticas}
%------------------------------------------------

\begin{frame}
	\frametitle{Consideraciones prácticas}
	\begin{itemize}
		\item ¿Cómo evaluar los resultados?
		\item ¿Cómo comparar arquitecturas?
		\item Mode collapse
	\end{itemize}
\end{frame}

%------------------------------------------------
\section{Conclusión}
%------------------------------------------------

\begin{frame}
\frametitle{Conclusión}
\begin{itemize}
	\item Campo de investigación en auge
	\item Interesantes aplicaciones en diversos ámbitos
	\item Difícil entrenamiento
	\item Difícil evaluación
	\item Consumo de grandes cantidades de recursos computacionales
	\item ¡Aún quedan muchas cosas por descubrir!
\end{itemize}
\end{frame}

%------------------------------------------------
\section{Referencias principales}
%------------------------------------------------

\begin{frame}
\frametitle{Referencias principales}
\begin{thebibliography}{9}
\bibitem{gan}
I. Goodfellow, J. Pouget-Abadie, M. Mirza, B. Xu, D. Warde-Farley, S. Ozair, A. Courville, and Y. Bengio. Generative adversarial nets. In \textit{Advances in neural information processing systems}, pages 2672--2680, 2014.

\bibitem{dcgan}
A. Radford, L. Metz, and S. Chintala. Unsupervised representation learning with deep convolutional generative adversarial networks. \textit{arXiv preprint arXiv:1511.06434}, 2015.

\bibitem{dl}
I. Goodfellow, Y. Bengio, and A. Courville. \textit{Deep Learning}. MIT Press, 2016. \texttt{http://www.deeplearningbook.org}.

\end{thebibliography}
\end{frame}

%------------------------------------------------

\begin{frame}
\centering
\Huge
Gracias por su atención
\end{frame}

\end{document}